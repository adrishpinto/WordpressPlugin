...
add_shortcode('activeloc_lang_switcher', function ($atts) {
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        currentFlag.addEventListener("click", function() { menu.classList.toggle("show"); });
        document.addEventListener("click", function(e) { if (!box.contains(e.target)) menu.classList.remove("show"); });
        ...
        if (savedLang) {
            ...
            var savedItem = Array.from(items).find(li => li.dataset.lang === savedLang);
            if (savedItem) currentFlag.textContent = savedItem.textContent;
        }
    });
    </script>';
    echo '<div id="activeloc-debug" style="margin-top:10px;font-size:12px;color:#666;"></div>';
